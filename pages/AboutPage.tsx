...
const AgentCard: React.FC<{ agent: Agent }> = ({ agent }) => (
  <div className="bg-white rounded-lg shadow-md overflow-hidden text-center p-6 transform hover:-translate-y-2 transition-transform duration-300">
    ...
    <h3 className="mt-4 text-xl font-bold text-brand-slate">{agent.name}</h3>
    <p className="text-brand-gold font-semibold">{agent.title}</p>
    <p className="mt-2 text-sm text-brand-slate/90">{agent.bio}</p>
  </div>
);
    ...
    <div className="bg-white">
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-bold text-brand-slate font-serif mb-4">Notre Histoire</h2>
              <p className="text-brand-slate/90 mb-4">{aboutData.history}</p>
              <p className="text-brand-slate/90">{aboutData.mission}</p>
            </div>
            <div>
      ...
      <section className="py-20 bg-brand-light">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-brand-slate font-serif">Notre Équipe d'Experts</h2>
            <p className="mt-4 text-lg text-brand-slate/90 max-w-2xl mx-auto">Des professionnels passionnés et dévoués à votre succès.</p>
          </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-brand-slate font-serif mb-4">{aboutData.interventionTitle}</h2>
          <p className="text-brand-slate/90 mb-8 max-w-3xl mx-auto">{aboutData.interventionText}</p>
