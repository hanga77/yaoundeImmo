    ...
    <Link to={`/bien/${property.id}`} className="block group">
      <div className="bg-white rounded-lg shadow-md overflow-hidden h-full flex flex-col transform group-hover:-translate-y-2 transition-transform duration-300">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <h3 className="text-lg font-semibold text-brand-slate mt-2 truncate">{property.title}</h3>
          <div className="flex items-center text-brand-slate/90 mt-1">
            <MapPinIcon className="h-5 w-5 mr-1" />
            <p className="text-sm">{property.address}, {property.commune}</p>
          </div>
          
          <div className="mt-4 pt-4 border-t border-gray-200 flex items-center space-x-4 text-sm text-brand-slate/90 flex-grow-0">
            {property.bedrooms > 0 && 
